...
def load_crawl_schedule():
    scheduler.remove_all_jobs()
    if os.path.exists(CRAWL_SETTINGS_FILE):
        with open(CRAWL_SETTINGS_FILE, "r") as f:
            data = json.load(f)
            ...
            for t in times:
                try:
                    hour, minute = t.split(":")
                    scheduler.add_job(
                        collector.run_crawl_cycle,
                        CronTrigger(hour=int(hour), minute=int(minute))
                    )
                    print(f"[Schedule] 크롤링 스케줄 등록 완료: 매일 {t}")
                except Exception as e:
                    print(f"[Schedule] 시간 파싱 오류 ({t}): {e}")

@asynccontextmanager
async def lifespan(app: FastAPI):
    # Initialize collector and Qdrant
...
async def trigger_crawl_now(background_tasks: BackgroundTasks):
    # 백그라운드로 즉시 크롤링 함수 호출
    background_tasks.add_task(collector.run_crawl_cycle)
    return {"success": True, "message": "즉각적인 크롤링 사이클 명령을 하달받았습니다."}

if __name__ == "__main__":
    import uvicorn
    print(f"[OK] Intelligence 통합 백엔드 실행 중")
